...
function bibleNativeSpeakChapter(startIndex) {
    ...
    const sep = en ? '.\n' : '。\n';
    // Android 原生 TTS 對單次朗讀的文字長度有上限（約 4000 字元），
    // 超過上限的內容會靜默失敗（例如 KJV 創世記第 1 章 = 4147 字元）。
    // 因此在經文邊界把整章切成多段，逐一排隊朗讀即可解決。
    const MAX_CHARS = 3800;

    const chunks = [];
    let cur = [], curLen = 0;
    segs.forEach(v => {
        const inc = v.length + 2; // 內容＋分隔符
        if (cur.length && curLen + inc > MAX_CHARS) {
            chunks.push(cur);
            cur = [];
            curLen = 0;
        }
    ...
    // 每節在整章文字中的起點（global offset），onRangeStart 用它高亮度「正在讀的經文」
    bibleSpeech.nativeOffsets = [];
    // 每段起點的 global offset，onRangeStart 回報的是「段內」起點，需加上段位移
    bibleSpeech.nativeChunkBases = [];
    let off = 0;
    let vidx = startIndex;
    chunks.forEach(chunk => {
        bibleSpeech.nativeChunkBases.push(off);
        chunk.forEach(v => {
            bibleSpeech.nativeOffsets.push({ verse: vidx, start: off });
            vidx++;
            off += v.length + 2;
        });
    });
    bibleSpeech.nativeChunkIdx = 0;
    bibleSpeech.nativeLastStart = -1;
    ...
    // 依序朗讀各段：第一段 Flush（取代目前播放），其餘 Add（排隊接續）
    ...
    chunks.forEach((chunk, ci) => {
        tts.speak({
            ...
            rate: 0.95,
            pitch: 1,
            queueStrategy: ci === 0 ? 0 : 1, // 0 = Flush, 1 = Add
            category: 'ambient',
            ...(vi >= 0 ? { voice: vi } : {})
        }).then(() => {
            remaining--;
    ...
    // 安全網：speak() 的 Promise 正常會在播完後 resolve（onDone），
    // 若裝置未回呼則以文字長度估算完成時間兜底（英文約 80ms/字、中文約 215ms/字）
    clearTimeout(bibleSpeech.nativeTimer);
    const msPerChar = en ? 80 : 215;
    const estMs = Math.max(3000, Math.round(off * msPerChar / 0.95)) + 8000;
...
if (bibleSpeech.native && window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.TextToSpeech) {
    window.Capacitor.Plugins.TextToSpeech.addListener('onRangeStart', info => {
        ...
        // 段落切換偵測：下一段的 onRangeStart 起點會從接近 0 重新開始
        const bases = bibleSpeech.nativeChunkBases || [];
        if (bases.length > 1
            && bibleSpeech.nativeLastStart >= 0
            && start <= bibleSpeech.nativeLastStart
            && bibleSpeech.nativeChunkIdx < bases.length - 1) {
            bibleSpeech.nativeChunkIdx++;
        }
        bibleSpeech.nativeLastStart = start;
        const abs = (bases[bibleSpeech.nativeChunkIdx] || 0) + start;
        let vi = offsets.length - 1;
        for (let i = 0; i < offsets.length; i++) {
            if (abs < offsets[i].start) { vi = i - 1; break; }
        }
