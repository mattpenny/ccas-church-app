...
export async function getDocuments(request, env) {
    ...
    try {
        ...
        if (category !== 'all') {
            query += isAuth ? ' WHERE category = ?' : ' AND category = ?';
...
export async function deleteDocument(request, env, params) {
    ...
    try {
        ...
        if (env.R2 && results[0].file_key) {
            await env.R2.delete(results[0].file_key);
        }
...
export async function downloadDocument(request, env, params) {
    ...
    try {
        ...
        const object = await env.R2.get(results[0].file_key);
